...
const getRoleBadge = (role: string) => {
    ...
    if (r.includes("admin")) return { bg: "bg-purple-100", text: "text-purple-700", label: "Administrator" };
    if (r.includes("police")) return { bg: "bg-blue-100", text: "text-blue-700", label: "Police Officer" };
    if (r.includes("tourism")) return { bg: "bg-green-100", text: "text-green-700", label: "Tourism Officer" };
                                ...
                                <>
                                    <div className="grid flex-1 text-left text-sm leading-tight">
                                        ...
                                        <span className="truncate text-xs text-muted-foreground">
                                            {user.email || `${user.name.toLowerCase().replace(/\s+/g, '.')}@gov`}
                                        </span>
                                    </div>
                            ...
                            <div className="flex items-center gap-3 p-3 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-t-xl">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <p className="truncate text-sm text-gray-600">
                                        {user.email || `${user.name.toLowerCase().replace(/\s+/g, '.')}@authority.gov`}
                                    </p>
                                </div>
                            </div>
                        </DropdownMenuLabel>
