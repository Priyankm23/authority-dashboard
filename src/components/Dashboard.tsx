          ...
          stats={[
            {
              ...
              changeType: (stats.sosAlertsToday.change.includes('+') && !stats.sosAlertsToday.change.includes('+0')) ? "negative" as const : "positive" as const,
              icon: AlertTriangle,
            ...
            },
            {
              ...
              change: avgResponse <= 15 
                ? `${((15 - avgResponse) / 15 * 100).toFixed(0)}% below SLA` 
                : `${((avgResponse - 15) / 15 * 100).toFixed(0)}% above SLA`,
              changeType: avgResponse <= 15 ? "positive" as const : "negative" as const,
              icon: Clock,
              helperText: avgResponse <= 15 ? "Within 15m SLA target" : "Exceeds 15m SLA target",
            },
            {
              id: "active-tourists",
              name: "Active Tourists",
              value: stats.activeTourists.count.toLocaleString(),
              change: stats.activeTourists.change === "+0.0% from yesterday" || stats.activeTourists.change === "+0% from yesterday" 
                ? "Stable" 
                : stats.activeTourists.change,
              changeType: stats.activeTourists.change.includes('+') && !stats.activeTourists.change.startsWith('+0') 
                ? "positive" as const 
            ...
            },
            {
              ...
              value: `${utilPercent}%`,
              change: utilPercent >= 80 ? "High" : utilPercent >= 50 ? "Moderate" : "Low",
              changeType: utilPercent >= 80 ? "negative" as const : utilPercent >= 50 ? "neutral" as const : "positive" as const,
              icon: Activity,
            ...
            },
            {
              ...
              change: stats.resolvedCases.change === "+0% from last month" || stats.resolvedCases.change === "+0.0% from last month"
                ? "No change"
                : stats.resolvedCases.change,
              changeType: stats.resolvedCases.change.includes('+') && !stats.resolvedCases.change.startsWith('+0') 
                ? "positive" as const 
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartSeries} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                  ...
                  <Tooltip
                    ...
                    formatter={(value: any) => [`${Number(value).toFixed(2)} min`, 'Response Time']}
                  />
